...
class mario(pygame.sprite.Sprite):
    ...
    def collideBricks(self, mario, listBricks):
        ubication = -1
        self.mario = mario
        self.listBricks = listBricks
        for brick in self.listBricks:
            if pygame.sprite.collide_mask(self.mario, brick):
                ubication = self.listBricks.index(brick)
        return ubication
    ...
    def jump(self, lvl):
        if lvl > 1:
            if self.down:
                ...
                if self.rect.collidelist(self.listB) > -1:
                    self.counter = 0
                    self.down = True
                else: self.posY += 30
